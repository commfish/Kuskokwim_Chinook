...
#'------------------------------------------------------------------------------
# RR Input data 
rr_data <- 'Kusko_Chinook_RR_Input_2024.csv'  
# Age Input data  (Used to create Brood table)
age_data <- 'Kusko_Chinook_RR_age_2024.csv'
# Used to make model 
...
nyear <- with(TMB.data, nyear)
nweir <- (TMB.data$nweir)
naerial <- (TMB.data$naerial)

#'Run default model ------------------------------------------------------------
#'------------------------------------------------------------------------------
#  Set Parameter initial, upper, and lower bounds                                     
#'------------------------------------------------------------------------------
# Original model + Salmon(Pitka) Weir 
...
rep <- sdreport(fit)  
# Save Likelihood 
makdata(fit$report(),file.path(Out_dir,'Chinook_RR_Likelihood_1.dat'))
# Extract Model Parameters and Save
...
rep <- sdreport(fit)
# Save Likelihood 
makedata(fit$report(),file.path(Out_dir,'Chinook_RR_Likelihood_3.dat'))
# Extract Model Parameters and Save
par1 <-as.data.frame(summary(rep))
names(par1)[2] <- 'Std.Error'
...
n <- 100
CV <- 0.3
jit <- data.frame(f = rep(0,n), N = rep(0,n))
for(i in 1:n){
parameters <- list(log_trun=rnorm(nyear,12.5,12.5*CV), log_wesc=rnorm(nweir,5.0,5*CV), 
log_aesc = rnorm(naerial,4,4*CV), log_cvw = log(0.5), log_cva=log(0.5),log_q = rep(-11,2), 
log_cvq = log(0.5),mq = 0.25, log_fl = rep(1.0, nyear), log_fu = rep(2.0,nyear))
fit <- runRTMB(TMB.data,Chinook_RR_Model_RTMB_New,parameters,p.upper,p.lower) 
jit$f[i] <- fit$report()$f
rep <- sdreport(fit)
par1 <-as.data.frame(summary(rep))
jit$N[i] <- par1[rownames(par1)==paste0('t_run.',(nyear-1)),1]
}
