...
Cpue.fig.s1 <- t(TMB.data$cpue)
Cpue.fig.s1[Cpue.fig.s1==0]<- NA
Cpue.fig.s2 <- t(TMB.data$cpue)
# Erase data: This section is used when you want to put different colors 
...
riv <- c('Upper','Middle','Lower','Mainstem','Commercial CPUE')
label.u <- up$Label
label.m <- mid$Label
label.l <- low$Label	 
...
plot(year,Esc.fig.u1[,1],xlim=c(min(year)-1,max(year+1)),ylim=c(0,sn.data),xaxs='i',yaxt='n',
       type="n",xlab="",ylab="",main="Kuskokwim River Chinook RR model data",cex.main=1.)  
st <- 0
for (i in 1:5){
